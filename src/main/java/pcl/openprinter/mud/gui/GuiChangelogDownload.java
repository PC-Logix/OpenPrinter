...
import pcl.openprinter.mud.ModUpdateDetector;
import pcl.openprinter.mud.UpdateChecker;
import pcl.openprinter.mud.UpdateEntry;
...
public class GuiChangelogDownload extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        for(int i= 0; i < bullets.length; i++){
            bulletWidth[i] = mc.fontRenderer.getStringWidth(bullets[i]+" ");
        }
    ...
    public void drawScreen(int p_571_1_, int p_571_2_, float p_571_3_)
    {
        ...
        if(isDownloading){
            ...
            drawCenteredString(mc.fontRenderer, StatCollector.translateToLocal("gui.downloading"), width/2, y + 15, 0xFFFFFF00);
            ...
            if(downloadComplete){
                drawCenteredString(mc.fontRenderer, StatCollector.translateToLocal("gui.download.complete"), width/2, y + 70, 0xFF44FF44);
            }

            if(downloadFailed){
                drawCenteredString(mc.fontRenderer, StatCollector.translateToLocal("gui.download.failed"), width/2, y + 70, 0xFFFF0000);
            }

            if(message != null){
                drawCenteredString(mc.fontRenderer, message, width/2, y + 85, 0xFFFFFFFF);
            }
    ...
    private int drawText(String line, int start) {
        ...
        if(line == null){
            ...
            this.drawString(mc.fontRenderer, main.replaceAll("=", "").trim(), (int)((startX)*scale), (int)(start*scale), 0xFFFFFF00);
            GL11.glScalef(scale, scale, scale);

            if(line.lastIndexOf("==")+2 <= line.length()){
                String sub = line.substring(line.lastIndexOf("==")+2, line.length()).trim();
                GL11.glScalef(1/scale, 1/scale, 1/scale);
                this.drawString(mc.fontRenderer, sub.replaceAll("=", "").trim(), (int)((startX+mc.fontRenderer.getStringWidth(main+"   "))*scale), (int)(start*scale), 0xFF2222FF);
                GL11.glScalef(scale, scale, scale);
            }
            ...
            float scale = 1.1F;
            GL11.glScalef(1/scale, 1/scale, 1/scale);
            this.drawString(mc.fontRenderer, line.replaceAll("\\*\\*", "").trim(), (int)((startX)*scale), (int)(start*scale), 0xFFFFFFFF);
            GL11.glScalef(scale, scale, scale);
            return (int)(1F/scale * 10)+start;
        }else{
            float scale = 1.2F;
            ...
            List<String> lineList = mc.fontRenderer.listFormattedStringToWidth(line, width - 10 - startX);
            ...
            for(int i = 0; it.hasNext(); i++){
                ...
                this.drawString(mc.fontRenderer, subline, (int) ((startX) * scale), (int) (start * scale), 0xFFFFFFFF);
