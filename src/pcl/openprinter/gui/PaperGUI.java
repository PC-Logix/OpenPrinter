...
public class PaperGUI extends GuiScreen {
	...
	for (int x = 0; x <= stack.stackTagCompound.getTags().size(); x++) {
		...
		if (alignment.equalsIgnoreCase("center")) {
			...
			fontRenderer.drawString(output, width/2 - fontRenderer.getStringWidth(output)/2, height / 2 - offset, color);	
		} else {
			fontRenderer.drawString(output , width/2 - xSizeOfTexture/2 + 6, height / 2 - offset, color);
		}
		offset = offset - 10;
	}
